...
var columns = [[
	//{field:'check',width:30,align:'center',title : '선택',checkbox : true},
	   // {field:'action',width:20,align:'center', halign : 'center',formatter : projectActionFormatter},
	    ...
	    {field:'depositAmount',width:55,align:'center',title : '입금 금액',  formatter : accTargetRangeFormatter},
	    {field:'depositStatus',width:40,align:'center',title : '입금 상태',  formatter : accTargetRangeFormatter},
	    {field:'uploadFile',width:55,align:'center',title : '적립 대상',  formatter : accTargetRangeFormatter},
...
function initView(){
	...
	$('#search_total_year_btn').linkbutton({
		onClick : function(){
			...
			returnp.api.call("selectPointCodeReports", param, function(res){
	...
	$('#search_btn').linkbutton({
		onClick : function(){
			...
			returnp.api.call("selectPeriodPointCodeReports", param, function(res){
				console.log(res);
				if (res.resultCode == "100") {
		 ...
		 onRowContextMenu : function(e, index, row){
			 ...
			 var item = null;
				cmenu.menu('appendItem', {
		  			...
		  			text:  "<strong>영수증 적립 코드 요청 상태 변경</strong>",
		  			//iconCls: 'icon-ok',
		  			onclick: function(){
		  			}
		  		});
				item = cmenu.menu('findItem', '영수증 적립 코드 요청 상태 변경');  
				cmenu.menu('appendItem', {
		  			parent: item.target,  // the parent item element
		  			//iconCls: 'icon-ok',
		  			text:  row.useStatus == "1" ? roundLabel("사용 가능", "#04B404") : "사용 가능",
		  			onclick: function(){
		  				changePointCodeStatus({pointCouponNo : row.pointCouponNo, useStatus : "1",couponNumber : row.couponNumber});
		  			}
		  		});
				...
				cmenu.menu('appendItem', {
		  			...
		  			onclick: function(){
		  				changePointCodeStatus({pointCouponNo : row.pointCouponNo, useStatus : "2",couponNumber : row.couponNumber});
		  			}
		  		});
				...
				cmenu.menu('appendItem', {
		  			parent: item.target,  // the parent item element
		  			//iconCls: 'icon-ok',
		  			text:  row.useStatus == "2" ? roundLabel("사용 완료", "#04B404") : "사용 완료",
		  			onclick: function(){
		  				changePointCodeStatus({pointCouponNo : row.pointCouponNo, useStatus : "2",couponNumber : row.couponNumber});
		  			}
		  		});
				...
				cmenu.menu('appendItem', {
		  			parent: item.target,  // the parent item element
		  			//iconCls: 'icon-ok',
		  			text:  row.useStatus == "2" ? roundLabel("등록 취소", "#04B404") : "등록 취소",
		  			onclick: function(){
		  				changePointCodeStatus({pointCouponNo : row.pointCouponNo, useStatus : "2",couponNumber : row.couponNumber});
		  			}
		  		});
				...
				cmenu.menu('appendItem', {
		  			parent: item.target,  // the parent item element
		  			//iconCls: 'icon-ok',
		  			text:  row.useStatus == "2" ? roundLabel("여러파일 업로드 - 처리 불가", "#04B404") : "여러파일 업로드 - 처리 불가",
		  			onclick: function(){
		  				changePointCodeStatus({pointCouponNo : row.pointCouponNo, useStatus : "2",couponNumber : row.couponNumber});
		  			}
		  		});
...
function changePointCodeStatus(options){
