		 ...
		 <div class="easyui-panel" title="" style="width:100%;padding:10px 15px;">
	        <form id="searchForm" method="post">
	             <div style="margin-bottom:10px;margin-left : 10px;display:inline-block;width:200px">
	                <select id = "searchAccountStatus" class="easyui-combobox" name="searchAccountStatus" label="상태" style="width:100%">
	                	<option value="0">전체</option>
	                	<c:forEach var="memberBankStatus" items="${memberBankStatusList}" varStatus="status">
						   <option value="${memberBankStatus.key}" >${memberBankStatus.value}</option>
						</c:forEach>
	                </select>
	            </div>
